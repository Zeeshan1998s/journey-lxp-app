...
fs.writeFileSync(path.join(__dirname, 'app/courses/page.tsx'), code, 'utf8');

// Update TopNavbar
const navPath = path.join(__dirname, 'app/components/TopNavbar.tsx');
let navContent = fs.readFileSync(navPath, 'utf8');

// Change the 'Courses' route to '/courses'
navContent = navContent.replace(
  \`if (item === 'Courses') href = '/courses/backend'; // default courses to backend for now\`,
  \`if (item === 'Courses') href = '/courses';\`
);

// We should also highlight the primary 'Courses' tab correctly in TopNavbar.
// the current TopNavbar logic for primary nav is: const isActive = pathname.startsWith(itemPath);
// Since /courses, /courses/backend, /courses/devops all start with /courses, "Courses" will be active.

fs.writeFileSync(navPath, navContent, 'utf8');
console.log('Built grid page and updated TopNavbar.');
